...
class SignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // it will help to check if user already exist then fetch its previous photo and name
         database.collection("users").document(auth.uid!!).get().addOnSuccessListener {
             if(it.exists()){
                 checkPermissionForImage()
                val  currentUser = it.toObject(User::class.java)!!

                 Picasso.get().load(currentUser.imageUrl).into(userImgView)
                 nameEt.setText(currentUser.name)
                 downloadUrl = currentUser.imageUrl
